...
class CPLineInfoController: UIViewController,goCtlDelegate,UIAlertViewDelegate {
    ...
    var singleLineType:Int!     //单程判断往程还是返程：0往程，1返程
...
//    var uasId:Int!          //通讯地址ID
            ...
            if (orgOrderId==nil){
                if (rang==1){
                    ...
                    if isSolicit != String(0) {
                        lineInfoView.isShowRecruit=true
                        getJoinLineData()       //招募中线路加载
                    }else{
                        lineInfoView.isShowRecruit=false
                        lineInfoLoad()
                        self.lineInfoView.singleLineType=singleLineType
                    }
                }else{
                    isSolicit=lineInfoDicW.objectForKey("isSolicit") as! String
                    if isSolicit != String(0) {
                ...
                self.setChangeOrderDic.setValue("order_detail", forKey: "action")
                self.setChangeOrderDic.setValue(self.orgOrderId, forKey: "orderId")
                self.getOrderInfo()      //获取需要改签的线路详情，获取乘客
                self.lineInfoLoad()      //获取新的线路详情
            }
    ...
    func commitOrder(){
        ...
        //获取通讯地址Id
        let uasId=self.lineInfoView.getUasId()
        ...
        if (self.lineInfoView.dropRowW == nil){
            ...
            if(rang==1){
                ...
                }else if singleLineType==1{
                    self.getOffStationW=(stationListAr[self.lineInfoView.dropRowW+1].objectForKey("stationId")!)
                    self.getOnStationW=stationListAr[0].objectForKey("stationId")
                }
                ...
                self.getOffStationF=stationListArBack[self.lineInfoView.dropRowF+1].objectForKey("stationId")
...
//                var filtered = str!.stringByReplacingOccurrencesOfString("\n", withString: "", options: NSStringCompareOptions.LiteralSearch, range: NSMakeRange(0, str!.length))
            ...
            lineInfoModel.getOrderId(Constants.userInfoURL, para: orderDic, success: { (orderId) in
    ...
    func getOrderInfo(){
        ...
        lineInfoModel.getOrderInfo(Constants.userInfoURL, para: setChangeOrderDic, success: { (orderInfo) in
    ...
    func lineInfoLoad(){
        ...
        lineInfoModel.lineInfoDataLoad(Constants.userInfoURL, para: lineInfoDicW, success: { (dateListAr, stationListAr, passengerListAr, lineInfoObj, model) in
            hud.hide(true)
            ...
            self.lineInfoView.gradeLb.text="推荐"+getGradeString(lineInfoGradeNum)+"乘坐，"+"全程\(antipateMileage)公里，预计\(antipateTime)分钟"
            for i in 0..<stationListAr.count{
                
                let stationValue=(stationListAr)[i].objectForKey("stationName") as! String
                stationAr.appendString("、"+stationValue)
                stationOptions.addObject(stationValue)
            }
            if self.singleLineType==0{
                stationOptions.removeLastObject()
            }else if self.singleLineType==1{
                stationOptions.removeObjectAtIndex(0)
            }
            
            stationAr.deleteCharactersInRange(NSMakeRange(0, 1))
...
//                hud.hide(true)
    ...
    func lineInfoLoad2(){
        ...
        let hud=MBProgressHUD.showHUDAddedTo(self.view, animated: true)
        //返线路
        lineInfoModel2.lineInfoDataLoad(Constants.userInfoURL, para: lineInfoDicF, success: { (dateListAr, stationListAr, passengerListAr, lineInfoObj, model) in
            hud.hide(true)
            let stationAr=NSMutableString()
            let stationOptions=NSMutableArray()
            let lineInfoGradeNum=(lineInfoObj.objectForKey("grade") as! String)
            ...
            self.lineInfoView.gradeBackValue.text="推荐"+getGradeString(lineInfoGradeNum)+"乘坐，"+"全程\(antipateMileage)公里，预计\(antipateTime)分钟"
            for i in 0..<stationListAr.count{
    ...
    func getJoinLineData(){
        weak var weakSelf=self
        let hud=MBProgressHUD.showHUDAddedTo(self.view, animated: true)
        lineInfoModel.getJoinLineInfo(Constants.userInfoURL, para: lineInfoDicW, success: { (success) in
            hud.hide(true)
    ...
    func comfirmChangeOrder(){
        ...
        weak var weakSelf=self
        lineInfoModel2.getOrderId(Constants.userInfoURL, para: setChangeOrderDic, success: { (orderId) in
...
//            let orderType = (lineDetailData?.objectForKey("isOrderGroup") as! Int) == 0 ? "1" : "2"
    ...
    func showAlert(){
        let alert=UIAlertView(title: "提示", message: "服务器连接失败，请稍后再试", delegate: self, cancelButtonTitle: "确定")
        alert.tag=22
        alert.show()
    }
    //点击确定跳转
