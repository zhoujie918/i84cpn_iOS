...
class SKScNavBar: UIView {
    ...
    var setViewWithItemIndex:Int {
        set{
            ...
            UIView.animateWithDuration(0.1) { () -> Void in
                //                self.itemsNum=CGFloat(self.items.count)
                //                let lineWidth = self.itemsWidth[self.currentItemIndex] as! CGFloat
                self.line.frame = CGRectMake(itemBtn.frame.origin.x + 2, self.line.frame.origin.y, kScreenWidth/CGFloat(self.itemsTitles.count), self.line.frame.size.height)
                for btn in self.items {
                    let tempBtn = btn as! UIButton
                    tempBtn.setTitleColor(UIColor(red: 102/255,green:102/255,blue:102/255,alpha:1), forState: UIControlState.Normal)
                    tempBtn.titleLabel?.font = UIFont.boldSystemFontOfSize(14)
                }
                itemBtn.setTitleColor(UIColor(red: 249/255, green: 202/255, blue: 48/255, alpha: 1), forState: UIControlState.Normal)
                itemBtn.titleLabel?.font = UIFont.boldSystemFontOfSize(14)
            }
        }
        get{
    ...
    private func getScNavContentAddScNavBarItemsWithItemsWidth(widths:NSArray) -> CGFloat {
        ...
        btn.setTitleColor(UIColor(red: 249/255, green: 202/255, blue: 48/255, alpha: 1), forState: UIControlState.Normal)
        btn.titleLabel?.font = UIFont.boldSystemFontOfSize(14)
        self.setLineWithItemsWidth(kScreenWidth/CGFloat(itemsTitles.count))
    ...
    private func setLineWithItemsWidth(width:CGFloat) {
        line = UIView(frame: CGRectMake(2, kScNavBarHeight - 2, width - 4, 2))
    ...
    //MARK: -- 通过Item上的标题获取每个Item的宽度组成的数组，即设置宽度
    private func getItemsWidthWithTitles(titles:NSArray) -> NSArray {
        let itemsWidths = NSMutableArray()
        for _ in titles {
            //            let str = title as! NSString
            //            let size:CGSize = str.sizeWithAttributes([NSFontAttributeName:UIFont.boldSystemFontOfSize(UIFont.systemFontSize())])
            let fValue:CGFloat = kScreenWidth/CGFloat(itemsTitles.count)
    ...
    func arrowBtnTapGesAction() {
        ...
        delegate?.isShowScNavBarItemMenu(false, height: height)
    }
