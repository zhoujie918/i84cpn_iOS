...
//    var statusView=CPOrderWaitView()
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if(orgOrderId != nil)&&(seatNumAr[selectedRow] as! NSObject==0) {
            ...
            let singleLineType:String!
            if singleTrip==1{
                singleLineType=lineData[selectedRow].objectForKey("lineType") as! String
            }else{
                singleLineType="100"
            }
            self.isRecruit=isRecruitAr[selectedRow] as! String
            if singleLineType != nil{
                self.delegate?.goLineInfoCtl(selectedRow,lineType: lineType,singleLineType:Int(singleLineType)!)
            }
