//
//  CPUnsubscribedViewController.swift
//  i84cpn
//
...
import UIKit
protocol UnsubscribedViewDelegate : NSObjectProtocol {
    func pushView(index:Int, orderId: Int)
}
class CPUnsubscribedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if (delegate != nil) {
            delegate?.pushView(3,  orderId: orderId)
        }
    }
    
    func getData() {
        CPAFHTTPSessionManager.postWithUrlString(Constants.userInfoURL, parameter: Constants.getOrderList(3), progressBlock: { (progress) in
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.routeLabel.text = (dataArray![indexPath.row].objectForKey("line_start_name") as! String) + "->" + (dataArray![indexPath.row].objectForKey("line_end_name") as! String)
    ...
    weak var delegate:UnsubscribedViewDelegate?
