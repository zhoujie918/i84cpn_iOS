...
import UIKit
protocol PaidViewDelegate : NSObjectProtocol {
    func pushView(index:Int, orderId: Int, status: Int)
}
class CPPaidViewController: CMBaseViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let iStatus = Int(status)!
        
        if (delegate != nil) {
            delegate?.pushView(2, orderId: orderId, status: iStatus)
        }
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let status = dataArray![indexPath.row].objectForKey("order_state") as! String
//        Constants.dPrint(status)
        cell.setStatus(atoi(status))
        return cell
    }
    weak var delegate:PaidViewDelegate?
