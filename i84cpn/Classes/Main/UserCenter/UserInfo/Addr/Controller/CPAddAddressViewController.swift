...
class CPAddAddressViewController: CMBaseViewController, UITextViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate, AreaDataModelDelegate {
    ...
    func pickerView(pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusingView view: UIView?) -> UIView {
        ...
        case 1:
            if areaDataModel.getCityNameArray().count > row {
                let provinceDic: Dictionary<String, String> = areaDataModel.getCityNameArray()[row] as! Dictionary<String, String>
                label!.text = provinceDic["dictName"]
            } else {
//                Constants.dPrint("cell  \(row)")
                label?.text = String(row)
            }
        case 2:
            if  areaDataModel.getCountyArray().count > row {
