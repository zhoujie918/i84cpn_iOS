...
+(void)uploadNewSuggestionWithUrlString:(NSString *)url params:(NSDictionary *)params ImageDataArray:(NSArray *)arrData progressBlock:(void (^)(NSProgress *))progress successBlock:(void (^)(id))success failureBlock:(void (^)(NSError *))failure{
    ...
    [[CPAFHTTPSessionManager shareHttpMangager]  POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
         ...
         */
        int i = 0;
        for (; i<arrData.count - 1; i++) {
            NSString *fileName = [[NSString alloc] initWithFormat:@"files%d", i];
            NSLog(@"%@", fileName);
            [formData appendPartWithFileData:arrData[i] name:fileName fileName:@"image.jpg" mimeType:@"image/jpg"];
        }
    } progress:^(NSProgress * _Nonnull uploadProgress) {
        if (progress) {
            progress(uploadProgress);
